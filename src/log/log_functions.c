...
__thread struct thread_log_context t_log_ctxt;
...
{
	struct thread_log_context *context = &t_log_ctxt;
	if (context->thread_name == NULL) {
		...
		if (ok_errors)
			LogFullDebug(COMPONENT_LOG_EMERG,
				"thread_log_context => %p", context);
	}
	return context;
}
...
{
	if (t_log_ctxt.thread_name != emergency_context.thread_name) {
		gsh_free(t_log_ctxt.thread_name);
		t_log_ctxt.thread_name = NULL;
	}
}
