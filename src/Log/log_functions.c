...
/* Current index for logfile rotation*/
                       ...
                       char                  * message)
{
  ...
  if(tab_log_flag[LF_LOGFILE_MAX_FILES].lf_ext == 0) {
    ...
    if(cur_logfile_indx >= tab_log_flag[LF_LOGFILE_MAX_FILES].lf_ext)
      cur_logfile_indx = 0;

    snprintf(fname, PATH_MAX, "%s.%02d.log", path, cur_logfile_indx);
  }
  if(compression) {
    strcat(fname, ".gz");
  }
  ...
  if(!compression)
    rc = write_to_file(fname, level, buffer, len);
  else
    rc = write_to_file_compressed(fname, level, buffer, len);
  if (rc == 0) {
    pthread_rwlock_wrlock(&logrot_rwlock);
    cur_logfile_size += len;
    pthread_rwlock_unlock(&logrot_rwlock);
  }
...
void SetLogFile(char * name, int reread)
{
  struct log_facility * facility;
  char                * tmp;
  ...
  /*
   * Since the log file is changed, reset the current size and index.
   */
  if (reread) {
    pthread_rwlock_wrlock(&logrot_rwlock);
    cur_logfile_size = 0;
  ...
  LogFullDebug(COMPONENT_LOG, "Changing log file to %s", name);
}
...
static int StrToSize(char * str_sz)
{
  int         nsize;
  int         len;
  char        tstr[PATH_MAX];
  ...
  if (len < 1)
    {
      LogWarn(COMPONENT_CONFIG, "Invalid LogFile Size specified\n");
      return -1;
    }
  if ((len + 1) > PATH_MAX)
    {
...
void reread_log_config()
{
  ...
  status = read_log_config(config_struct, TRUE);
  if(status < 0)
    {
      LogCrit(COMPONENT_CONFIG,
